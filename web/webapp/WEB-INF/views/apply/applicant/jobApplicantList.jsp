...
<!-- 						<tr> -->
<!-- 							<td>2</td> -->
<%-- 							<td><a href="${cPath }/applicant/com/applicantManList.do">채용 공고 지원</a></td> --%>
<!-- 							<td>2022.02.12</td> -->
<!-- 							<td>2022.03.20</td> -->
<!-- 							<td>디자인</td> -->
<!-- 						</tr> -->
...
let listBody1 = $("#listBody1").on("click", "tr", function(){
	let jobapp = $(this).data("jobapp");
	if(jobapp)
		location.href = CONTEXTPATH+"/applicant/com/jobApplicantManList.do?what="+jobapp.jobSn;
});
...
let searchForm1 = $("#searchForm1").ajaxForm({
	...
	, success:function(resp){
		...
		pagingArea1.empty();
		$("[name=page]").val("");
		let jobappList = resp.list;
		let pagingHTML = resp.pagingHTMLBS;
		let trTags = [];
		if(jobappList && jobappList.length > 0){
			$.each(jobappList, function(index, jobapp){
				let trTag = $("<tr>");
				if(new Date(jobapp.jobEndd) > new Date(getToday)){  // 여기가 뭔가 이상함 - 재대로 먹히지 않음
					trTag.append(
						$("<td>").html(jobapp.rnum)
						, $("<td>").html(jobapp.jobTitle)
						, $("<td>").html(jobapp.jobStartd)
						, $("<td>").html(jobapp.jobEndd)
						, $("<td>").html(jobapp.jobDeptCd)
					).data("jobapp", jobapp);
				};
				trTags.push(trTag);
			});
