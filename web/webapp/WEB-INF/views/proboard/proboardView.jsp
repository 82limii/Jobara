		...
		<table class="table">
						...
						<td class="condition">
							<table class="table">
								...
								<tr>
									<td>지역</td>
									<c:choose>
										<c:when test="${proBoard.probCity eq proBoard.probLoc}">
											<td>${proBoard.probLoc }</td>
										</c:when>
										<c:otherwise>
											<td>${proBoard.probCity } ${proBoard.probLoc }</td>
										</c:otherwise>
									</c:choose>
								</tr>								
								<tr>
						...
						<td class="condition">
							<table class="table">
								...
								<tr>
									...
									<td><button class="btn btn-outline-secondary linkBtn" data-url="${cPath }/proboard/myProBoard.do?what=${proBoard.ememId}">기업공고</button>
									</td>
									<td>
									...
									<td colspan="3">
										<c:choose>
											<c:when test="${authMember.userType eq 'Emember'}">
												<c:if test="${authMember.ememId eq proBoard.ememId }">
													<button class="btn btn-primary linkBtn" data-url="${cPath }/proboard/com/proBoardUpdate.do?what=${proBoard.probSn}">수정</button>
												</c:if>
												<button class="linkBtn btn btn-outline-success" data-url="${cPath }/proboard/com/proBoardList.do">목록</button>
											</c:when>
											<c:otherwise>
												<button class="linkBtn btn btn-outline-warning" data-url="${cPath }/scrap/ScrapInsert.do?what=${proBoard.probSn}" >스크랩</button>
												<button class="linkBtn btn btn-outline-success" data-url="${cPath }/proboard/proBoardList.do">목록</button>
											</c:otherwise>
										</c:choose>
									</td>
								</tr>								
									...
									<td colspan="2">
										<c:if test="${authMember.userType ne 'Emember' }">
											...
											<c:if test="${fn:contains(way, '홈페이지') }">
												<button class="btn btn-outline-primary linkBtn" onclick="location.href='${proBoard.probPage}'">홈페이지 지원</button>
											</c:if>
			...
			<td>
				<table class="table">
					...
					<tr>               
						<td class="col-4">실시간 지원자 수</td>
						<td class="col-4">학력별</td>
						<td class="col-4">경력별</td>
					</tr>
					<tr>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="${cPath }/apply/proApply.do" method="post" id="applyForm">
...
<!-- 	        <input type="text" name="reSn" /> -->
	        <input type="hidden" name="probSn" value="${proBoard.probSn }" hidden="true"/>
