...
<c:set value="${requestScope['javax.servlet.forward.servlet_path'] }" var="servletPath"/>
	<div class="container-scroller">
		...
		<div class="container-fluid page-body-wrapper">
			<sidebar>
			<c:if test="${param.what ne null }">
				<c:choose>
					<c:when test="${fn:contains(servletPath,'com') }">
						<tiles:insertAttribute name="sidebarComPro" />
					</c:when>
					<c:otherwise>
						<tiles:insertAttribute name="sidebarPro" />
					</c:otherwise>
				</c:choose>
			</c:if>
			<c:if test="${param.what eq null }">
				<tiles:insertAttribute name="sidebarCommonPro"></tiles:insertAttribute>
			</c:if>
			</sidebar>
